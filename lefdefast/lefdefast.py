...
_def_parse_schema = {
    ...
    "DESIGN_ITEM": _CHOICE("ROW_CMD", "TRACKS_CMD", "VIAS", "COMPONENTS", "PINS", "BLOCKAGES", "SPECNETS"),
    ...
    "ROW_CMD": _STRUCT(_SEQ(_KEY("ROW"), _VAR(str, "Name"), _VAR(str, "Site"),
                       _VAR(_ARRAY(int, 2), "Origin"), _VAR(str, "Orient"),
                       _OPT(_SEQ(_KEY("DO"), _VAR(int, "NX"), _KEY("BY"), _VAR(int, "NY"),
                       _OPT(_SEQ(_KEY("STEP"), _VAR(int, "DX"), _VAR(int, "DY"))))), _END_CMD, Element="Row")),
    ...
    "HTRACKS": _SEQ(_KEY("X"), _VAR(int, "X"),
                    ...
                    _OPT("MASK"), _OPT("LAYER"),
                    Element="Tracks", Direction="Vertical", Y=0),
    "VTRACKS": _SEQ(_KEY("Y"), _VAR(int, "Y"),
                    _KEY("DO"), _VAR(int, "Number"), _KEY("STEP"), _VAR(int, "DX"),
                    _OPT("MASK"), _OPT("LAYER"),
                    Element="Tracks", Direction="Vertical", Y=0),
    "MASK": _SEQ(_KEY("MASK"), _VAR(int, "MaskNum"), _OPT(_KEY("SAMEMASK"), SameMask=True)),
    "LAYER": _SEQ(_KEY("LAYER"), _VAR(_ARRAY(str), "Layers")),
    "VIAS": _STRUCT(_SEQ("VIAS_CMD", _VAR(_ARRAY("VIA_CMD"), "Items"), "ENDVIAS_CMD", Element="Vias")),
    ...
    "RECT": _STRUCT(_SEQ(_KEY("+"), _KEY("RECT"), _VAR(str, "Layer"),
                    _OPT("MASKNUM"), _VAR(_ARRAY("XY", 2), "Points"), Shape="Rect")),
    "MASKNUM": _SEQ(_KEY("+"), _KEY("MASK"), _VAR(int, "MaskNum")),
    "POLYGON": _STRUCT(_SEQ(_KEY("+"), _KEY("POLYGON"), _VAR(str, "Layer"),
                       _OPT("MASKNUM"), _VAR(_ARRAY("XY"), "Points"), Shape="Polygon")),
    "COMPONENTS": _STRUCT(_SEQ("COMPONENTS_CMD", _VAR(_ARRAY("COMPONENT_CMD"), "Items"),
                               "ENDCOMPONENTS_CMD", Element="Components")),
    ...
    "PINS": _STRUCT(_SEQ("PINS_CMD", _VAR(_ARRAY("PIN_CMD"), "Items"), "ENDPINS_CMD", Element="Pins")),
    ...
    "PIN_CMD": _STRUCT(_SEQ(_KEY("-"), _VAR(str, "Name"), _KEY("+"), _KEY("NET"), _VAR(str, "Net"),
                            ...
                            _OPT(_VAR(_ARRAY(_STRUCT(_CHOICE(
                                            _SEQ(_KEY("+"), _KEY("PORT"), Element="Port"),
                                            _SEQ(_KEY("+"), _KEY("LAYER"),_VAR(str, "Layer"),
                                                 _OPT(_SEQ(_KEY("MASK"), _VAR(int, "MaskNum"))),
    ...
    "BLOCKAGES": _STRUCT(_SEQ("BLOCKAGES_CMD",
                              _VAR(_ARRAY(_CHOICE("BLOCKAGE_LAYER_CMD","BLOCKAGE_PLACEMENT_CMD")), "Items"),
                               "ENDBLOCKAGES_CMD", Element="Blockages")),
    ...
    "BLOCKAGE_LAYER_CMD": _STRUCT(_SEQ(_KEY("-"), _KEY("LAYER"), _VAR(str, "Layer"),
                                       _REPEAT(_SEQ(_KEY("+", _CHOICE(
                                                            ...
                                                            _SEQ(_KEY("MASK"), _VAR(int, "MaskNum")),
                                                            )))
                                          ), _VAR(_ARRAY(_STRUCT(_CHOICE(
                                                     _SEQ(_KEY("RECT"), _VAR(_ARRAY("XY", 2), "Points"), Shape="Rect"),
                                                     _SEQ(_KEY("POLYGON"), _VAR(_ARRAY("XY"), "Points"), Shape="Polygon")
                                                                ))), "Shapes"), _END_CMD)),
    ...
    "SPECNETS": _STRUCT(_SEQ("SPECNETS_CMD", _VAR(_ARRAY("SPECNET_CMD"), "Items"),
                             "ENDSPECNETS_CMD", Element="SpecNet")),
    ...
    "SPECWIRE_SHAPES": _SEQ(_REPEAT(_CHOICE(_SEQ(_KEY("+"), _KEY("SHAPE"), _VAR(str, "ShapeType")),
                            ...
                            _VAR(_ARRAY(_STRUCT(_CHOICE(
                                        _SEQ(_KEY("+"), _KEY("POLYGON"), _VAR(_ARRAY("XY"), "Points"), Shape="Polygon"),
                                        _SEQ(_KEY("+"), _KEY("RECT"), _VAR(_ARRAY("XY",2), "Points"), Shape="Rect"),
                                        _SEQ(_KEY("+"), _KEY("VIA"), _VAR(str, "Name"),
                                             _OPT(_VAR(str, "Orient")), _VAR(_ARRAY("XY", 1), "Points"), Shape="Via"),
                                        )), "NonEmpty"), "Shapes")
                           ), 
